...
状态：进行中（3.1~3.5 理论已完成，实操进行中：3-P1 环境搭建进行中）
...
- **已完成：** 4.1.1 GPIO基础（8模式/寄存器/初始化流程）、4.1.2 GPIO输出（LED/推挽开漏/BSRR）、4.1.3 GPIO输入（按键/上下拉/消抖）、4.1.4 复用功能（AF编号/映射表/AFRL/AFRH）、4.1.5 矩阵键盘扫描（逐行拉低/消抖）、4.1.6 EXTI外部中断（触发方式/NVIC配置/中断服务函数/回调）、4.1.7 中断编程实践（按键中断+LED翻转/多中断/volatile/安全写法）、4.1.8 GPIO实际应用（状态机LED/Bit-banging/唤醒/驱动外部电路）
...
| 2026-05-29 | 3-P1进行中 | DAP调试器调通（ATK-FS-HID-CMSIS-DAP，需原生HID驱动）；OpenOCD+CMSIS-DAP+SWD成功连接STM32MP157（检测到A7×2+M4）；下载STM32CubeMP1 SDK；讲解工程文件结构/启动文件/链接脚本/交叉编译概念；创建lab/01-cross-compile |
